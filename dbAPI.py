...
get_services_query = "SELECT services FROM user_platforms WHERE acctID = ':acctID'"
...
def update_services(user_id: str, service: str):
    with sshtunnel.SSHTunnelForwarder(
        ...
        userQuery = get_services_query.replace(":acctID", user_id)
...
def create_user(username: str, password: str, firstname: str, lastname: str):
    with sshtunnel.SSHTunnelForwarder(
        ...
        return {"username": username, "password": password, "firstname": firstname, "lastname": lastname, "id": maxID+1}
